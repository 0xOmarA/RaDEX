...
blueprint! {
    ...
    struct RaDEX {
        ...
        tracking_token_address_pair_mapping:
    ...
    impl RaDEX {
        ...
        pub fn pool_exists(&self, address1: ResourceAddress, address2: ResourceAddress) -> bool {
            ...
            let sorted_addresses: (ResourceAddress, ResourceAddress) =
                sort_addresses(address1, address2);
        ...
        pub fn new_liquidity_pool(&mut self, token1: Bucket, token2: Bucket) -> Bucket {
            ...
            let (liquidity_pool, tracking_tokens): (ComponentAddress, Bucket) =
                LiquidityPool::new(bucket1, bucket2, dec!("0.3"));

            // Adding the liquidity pool to the hashmap of all liquidity pools
            self.liquidity_pools
                .insert(addresses, liquidity_pool.into());
            ...
            self.tracking_token_address_pair_mapping
                .insert(tracking_tokens.resource_address(), addresses);
        ...
        pub fn add_liquidity(
            ...
            let optional_liquidity_pool: Option<&LiquidityPool> =
                self.liquidity_pools.get(&addresses);
            match optional_liquidity_pool {
                Some(liquidity_pool) => {
                    ...
                    let returns: (Bucket, Bucket, Bucket) =
                        liquidity_pool.add_liquidity(bucket1, bucket2);
                    (Some(returns.0), Some(returns.1), returns.2)
                }
                None => {
                    // If this matches then there does not exist a liquidity pool for this token pair
        ...
        pub fn remove_liquidity(&mut self, tracking_tokens: Bucket) -> (Bucket, Bucket) {
            ...
            assert!(
                self.tracking_token_address_pair_mapping
                    .contains_key(&tracking_tokens.resource_address()),
        ...
        pub fn swap(&mut self, tokens: Bucket, output_resource_address: ResourceAddress) -> Bucket {
            ...
            let sorted_addresses: (ResourceAddress, ResourceAddress) =
                sort_addresses(tokens.resource_address(), output_resource_address);
        ...
        pub fn swap_exact_tokens_for_tokens(
            ...
            return self.liquidity_pools[&sorted_addresses]
                .swap_exact_tokens_for_tokens(tokens, min_amount_out);
        }
        ...
        pub fn swap_tokens_for_exact_tokens(
            ...
            self.assert_pool_exists(
                tokens.resource_address(),
                output_resource_address,
                String::from("DEX Swap For Exact"),
            );
