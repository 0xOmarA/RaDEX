...
blueprint! {
    ...
    impl LiquidityPool {
        ...
        pub fn new(
            ...
            assert_ne!(
                borrow_resource_manager!(token2.resource_address()).resource_type(),
                ResourceType::NonFungible,
                "[Pool Creation]: Both assets must be fungible."
            );
            ...
            let addresses: (ResourceAddress, ResourceAddress) =
                (bucket1.resource_address(), bucket2.resource_address());
        ...
        pub fn other_resource_address(&self, resource_address: ResourceAddress) -> ResourceAddress {
            ...
            self.assert_belongs_to_pool(
                resource_address,
                String::from("Other Resource ResourceAddress"),
            );

            // Checking which of the addresses was provided as an argument and returning the other address.
            let addresses: Vec<ResourceAddress> = self.addresses();
            return if addresses[0] == resource_address {
                addresses[1]
            } else {
                addresses[0]
            };
        ...
        pub fn calculate_output_amount(
            ...
            let y: Decimal =
                self.vaults[&self.other_resource_address(input_resource_address)].amount();
        ...
        pub fn calculate_input_amount(
            ...
            let x: Decimal =
                self.vaults[&self.other_resource_address(output_resource_address)].amount();
        ...
        pub fn add_liquidity(
            ...
            let (amount1, amount2): (Decimal, Decimal) =
            ...
            let tracking_tokens_manager: &ResourceManager =
                borrow_resource_manager!(self.tracking_token_address);
            ...
            let tracking_tokens: Bucket = self
                .tracking_token_admin_badge
                .authorize(|| tracking_tokens_manager.mint(tracking_amount));
            info!(
                "[Add Liquidity]: Owed amount of tracking tokens: {}",
                tracking_amount
            );
        ...
        pub fn remove_liquidity(&mut self, tracking_tokens: Bucket) -> (Bucket, Bucket) {
            ...
            let percentage: Decimal =
                tracking_tokens.amount() / tracking_tokens_manager.total_supply();
            ...
            let addresses: Vec<ResourceAddress> = self.addresses();
            let bucket1: Bucket = self.withdraw(
                addresses[0],
                self.vaults[&addresses[0]].amount() * percentage,
            );
            let bucket2: Bucket = self.withdraw(
                addresses[1],
                self.vaults[&addresses[1]].amount() * percentage,
            );
        ...
        pub fn swap(&mut self, tokens: Bucket) -> Bucket {
            ...
            let output_amount: Decimal =
                self.calculate_output_amount(tokens.resource_address(), tokens.amount());
        ...
        pub fn swap_exact_tokens_for_tokens(
            ...
            assert!(
                output_tokens.amount() >= min_amount_out,
                "[Swap Exact]: min_amount_out not satisfied."
            );
        ...
        pub fn swap_tokens_for_exact_tokens(
            ...
            info!(
                "[Swap For Exact]: Amount gievn out: {}",
